...
fromRecording mr =
    let
        ...
        linesWithTempo =
            distributeTempo header.formatType lines
    in
...
eventToNotable ( t, e ) =
    case e of
        NoteOn chanel pitch velocity ->
            ( t, Note pitch velocity )

        Tempo x ->
            ( t, MicrosecondsPerBeat x )
...
accumulateTimes =
    filterEvents << List.reverse << List.foldl accum []
